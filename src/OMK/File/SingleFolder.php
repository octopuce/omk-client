...
class OMK_File_SingleFolder extends OMK_File_Adapter {
    ...
    public function __construct($options) {
        if(array_key_exists("storage_path", $options) && null != $options["storage_path"]){
            ...
            throw new OMK_Exception("Missing storage path.",1);
        }
    }
    ...
    public function create($options){
        ...
        if( !is_dir($new_path)){
            ...
            if( !mkdir($new_path, 0777, TRUE) ){
                return array(
                    ...
                    "message"   => "Failed to create storage path."
                );
            }
        }
        if( !is_writable($new_path)){
            return array(
                ...
                "message"   => "Failed to access storage path."
            );
        }
        $new_path .= "/".$file_name;
        ...
        return array(
            "code"      => 0,
            "message"   => "File {$file_name} moved to file system"
        );
    }
} 
